...
type JointDispatchHandler struct{ service service.JointDispatchService }

func NewJointDispatchHandler(s service.JointDispatchService) *JointDispatchHandler {
	return &JointDispatchHandler{service: s}
}

func (h *JointDispatchHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/joint-dispatches")
	resource.GET("", h.list)
...
func (h *JointDispatchHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *JointDispatchHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *JointDispatchHandler) create(c *gin.Context) {
	var input dto.CreateJointDispatchOrder
...
func (h *JointDispatchHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateJointDispatchOrder
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *JointDispatchHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *JointDispatchHandler) remove(c *gin.Context) {
