...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	jointDispatchRepository := repository.NewJointDispatchRepository(db)
	...
	jointDispatchService := service.NewJointDispatchService(jointDispatchRepository, gateUnitRepository, operationDirectiveRepository, securityService)
	...
	jointDispatchHandler := handler.NewJointDispatchHandler(jointDispatchService)
	systemHandler := handler.NewSystemHandler(securityService, reservoirService, gateUnitService, operationDirectiveService, executionConfirmationService, jointDispatchService, db, redisClient)
	...
	jointDispatchHandler.Register(api)
